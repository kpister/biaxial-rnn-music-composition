...
def initial_state(layer, dimensions=None):
    ...
    """
    if not has_hidden(layer):
        return None

    if not dimensions:
        return layer.initial_hidden_state

    return at.repeat(at.shape_padleft(layer.initial_hidden_state), dimensions, axis=0)
...
class Model:
    def __init__(self, t_layer_sizes, p_layer_sizes, dropout=0):
        ...
        self.pitch_model.layers.append(
            Layer(p_layer_sizes[-1], 2, activation=at.sigmoid)
        )

        self.dropout = dropout

        self.conservativity = at.fscalar()
        self.srng = RandomStream(np.random.randint(0, 1024))
    ...
    def setup_train(self):
        ...
        start_note_values = at.alloc(
            np.array(0, dtype=np.int8), 1, time_final.shape[1], 2
        )
        correct_choices = (
            self.output_mat[:, 1:, 0:-1, :]
            .transpose((2, 0, 1, 3))
            .reshape((n_note - 1, n_batch * n_time, 2))
        )
        note_choices_inputs = at.concatenate(
        ...
        note_inputs = at.concatenate([time_final, note_choices_inputs], axis=2)
        num_timebatch = note_inputs.shape[1]

        # apply dropout
        if self.dropout > 0:
            pitch_masks = MultiDropout(
                [(num_timebatch, shape) for shape in self.p_layer_sizes], self.dropout
            )
        else:
            pitch_masks = []
        ...
        # (b,x,y,1) instead
        active_notes = at.shape_padright(self.output_mat[:, 1:, :, 0])
        mask = at.concatenate([at.ones_like(active_notes), active_notes], axis=3)

        loglikelihoods = mask * at.log(
        ...
        self.update_thought_fun = aesara.function(
            inputs=[self.input_mat, self.output_mat],
    ...
    def _predict_step_note(self, in_data_from_time, *states):
        ...
        in_data = at.concatenate([in_data_from_time, in_data_from_prev])
        ...
        chosen = at.cast(at.stack(shouldPlay, shouldArtic), "int8")
    ...
    def setup_predict(self):
        ...
        self.predict_seed = at.bmatrix()
        self.steps_to_simulate = at.iscalar()
        ...
        def step_time(*states):
            ...
            start_note_values = at.alloc(np.array(0, dtype=np.int8), 2)
            ...
            notes_result, updates = aesara.scan(
                fn=self._predict_step_note,
            ...
            # next_input = at.cast(at.alloc(0, 3, 4),'int64')
        ...
        time_result, updates = aesara.scan(
        ...
        self.predict_fun = aesara.function(
        ...
        self.predict_thought_fun = aesara.function(
            inputs=[self.steps_to_simulate, self.conservativity, self.predict_seed],
    ...
    def setup_slow_walk(self):
        ...
        self.walk_input = aesara.shared(np.ones((2, 2), dtype="int8"))
        self.walk_time = aesara.shared(np.array(0, dtype="int64"))
        self.walk_hiddens = [
            aesara.shared(np.ones((2, 2), dtype=aesara.config.floatX))
            for layer in self.time_model.layers
            if has_hidden(layer)
        ]
        ...
        next_input = OutputFormToInputFormOp()(
            output, self.walk_time + 1
        )  # TODO: Fix time
        # next_input = at.cast(at.alloc(0, 3, 4),'int64')
        ...
        updates.update(
            {self.walk_time: self.walk_time + 1, self.walk_input: next_input}
        )
        ...
        self.slow_walk_fun = aesara.function(
    ...
    def start_slow_walk(self, seed):
        ...
        for layer, hidden in zip(
            (l for l in self.time_model.layers if has_hidden(l)), self.walk_hiddens
        ):
            hidden.set_value(
                np.repeat(
                    np.reshape(layer.initial_hidden_state.get_value(), (1, -1)),
                    num_notes,
                    axis=0,
                )
            )
