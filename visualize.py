...
def actToColor(memcell, activation):
    return [0, at.sigmoid(activation), at.sigmoid(memcell)]
...
def thoughtsAndPastToStackedArray(thoughts, probs, succs, len_past):
    ...
    if len(probs) < len_past:
        past_out = np.pad(
            past_out, ((0, 0), (len_past - len(probs), 0), (0, 0)), mode="constant"
        )
    ...
    rows = [
        np.pad(row, ((0, 0), (maxlen - row.shape[1], 0), (0, 0)), mode="constant")
        for row in rows
    ]
